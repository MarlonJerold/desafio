...
<%
	if ( parameterBean.getSelectionList( ) != null )
	{
		if( !parameterBean.isRequired( ) || ( parameterBean.isCascade( ) && DataUtil.trimString( defaultValue ).length( )<=0 ) )
		{
			...
			{
%>
	    <!-- Não aparecer valor vazio no select	<OPTION SELECTED></OPTION> -->
<%				
			}
			else
			{
%>
		<!--Não aparecer valor vazio no select <OPTION></OPTION> -->
<%
			}
		}
...
<%	
		...
		//for ( int i = 0; i < parameterBean.getSelectionList( ).size( ); i++ ) // for começa de 1 p/ não mostrar NULL VALUE
		for ( int i = 1; i < parameterBean.getSelectionList( ).size( ); i++ )
		{
